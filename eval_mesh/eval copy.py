...
if __name__ == '__main__':
    mp.freeze_support()
    # "python {script_dir}/eval.py --data {result_mesh_file} --scan {scan} --mode mesh --dataset_dir {Offical_DTU_Dataset} --vis_out_dir {out_dir}"
    parser = argparse.ArgumentParser()
    parser.add_argument('--data', type=str, default='D:\2d-gaussian-splatting\output\ec536168-0\train\ours_30000\fuse_unbounded_post.ply')
    # parser.add_argument('--scan', type=int, default=1)
    parser.add_argument('--mode', type=str, default='mesh', choices=['mesh', 'pcd'])
    parser.add_argument('--gt_dir', type=str, default='D:\Free3D\MVS_data\render_res\LEGO_Duplo_Build_and_Play_Box_4629\mesh\meshes\model.obj')
    ...
    # load pose and mask
    cull_scan(args.gt_dir, ply_file, args.data, instance_dir=os.path.join(args.mask_dir, f'scan{args.scan_id}'))
    ...
    # load gt mask
    obs_mask_file = loadmat(f'{args.dataset_dir}/ObsMask/ObsMask{mask}_10.mat')
    ...
    stl_pcd = o3d.io.read_point_cloud(f'{args.dataset_dir}/Points/stl/stl{gt}_total.ply')
